...
describe("personal corpus WAV inspection", () => {
  ...
  it("rejects silence, clipping, invalid format, and bad duration", () => {
    expect(() =>
      inspectCapturedPcmWav(createWav(Array.from({ length: 16_000 }, () => 0))),
    ).toThrow(/silence/iu);
    expect(() =>
      inspectCapturedPcmWav(
        createWav(Array.from({ length: 16_000 }, () => 32_767)),
      ),
    ).toThrow(/clipping/iu);
    expect(() =>
      inspectCapturedPcmWav(
    ...
    expect(() =>
      inspectCapturedPcmWav(
        createWav(Array.from({ length: 1_000 }, () => 500)),
      ),
    ).toThrow(/duration/iu);
  });
});

function createWav(samples: readonly number[], sampleRate = 16_000): Buffer {
