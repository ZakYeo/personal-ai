...
function createIntentInstructions(
  ...
  return [
    ...
    ...(clarification?.origin === "confirmation_correction"
      ? [
          "This is a correction to an unexecuted prepared action. In this mode, command or plan parameters contain only changed fields; the application retains all omitted fields. Use null only to explicitly remove a field. Preserve every prepared step in its original position and capability, using an empty parameters array for an untouched step. For a single prepared step use command; for multiple steps use plan with exactly the same number of steps. Do not add reads, actions, or steps. When changing mutually exclusive input forms, explicitly remove the old form. Never infer that any action has executed. These correction rules override the normal complete-command instructions. For a missing replacement value in one declared field, use clarification with that field and the partial changed parameters. Use rephrase only when the change is too unclear to name a field; for a changed topic, return replacement. Only the application can revalidate and request fresh confirmation.",
        ]
      : []),
  ].join(" ");
}
