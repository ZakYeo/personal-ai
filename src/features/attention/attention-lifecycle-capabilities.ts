...
export function createAttentionLifecycleCapabilities(store: AttentionStore) {
  return {
    "attention.rules.list": defineCapability({
      ...
      description:
        "Read up to 24 user-authored attention rules with exact opaque IDs and revisions. No monitor is inferred or enabled by listing.",
      ...
      execute: async () => {
        ...
        return {
          data: { dailyBudget: state.preferences.dailyBudget, ...references },
          toolObservationData: {
            dailyBudget: state.preferences.dailyBudget,
            ...references,
          },
          text: state.rules.length
            ? state.rules
                .map(
                  (rule) =>
                ...
                .join(" ")
            : "You have no proactive attention rules enabled.",
        };
      },
    }),
