...
- Milestone 8.1 is implemented after its required independent maintainability
  ...
  Milestone 17 daily briefings are implemented after independent review. The
  active roadmap now commits Milestones 18 through 25: desktop presence and
  command center, voice interruption and responsiveness,
...
Milestones 18.1 and 19 now implement daily-use reliability, honest desktop state,
...
misclassified as failed and adds a 250 ms silent lead-in so WSLg can activate
the PulseAudio sink before speech begins.
...
On WSL, native desktop commands default to X11 window handling and software
rendering and disable
...
`GDK_BACKEND`, `LIBGL_ALWAYS_SOFTWARE`, `WEBKIT_DISABLE_DMABUF_RENDERER`, and
