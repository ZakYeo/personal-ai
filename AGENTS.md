...
- Personal profile state must contain only explicit user-authored facts in its first milestone, preserve provenance, support explanation/correction/deletion, and expose narrow requested fields rather than injecting the whole profile into providers or features.
...
- Weather answers and watches must preserve exact location, timezone, units, forecast period, and freshness data. A stored home location must be explicit, and weather watches are convenience notifications rather than guaranteed emergency alerts.
