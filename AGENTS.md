...
- The current product milestone is alarm usability and lifecycle controls: snooze, daily/weekly recurrence with an explicit IANA timezone, rescheduling, label editing, and human status are implemented; add retention serialized with scheduler observation and covered by deterministic restart tests.
