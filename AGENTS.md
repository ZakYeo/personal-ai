...
- Reserve `AssistantResponse.expectsFollowUp` for a reply required to finish the current interaction; optional result references must remain available across a later wake-word turn without keeping the microphone open.
...
- Keep the built-in capability catalog list and description commands terminal-only; their results are final human-facing answers, not intermediate provider observations.
...
- Normal capability-list requests group enabled capabilities through feature-level spoken summaries; enumerate capability-level summaries only when the decoded request explicitly asks for a complete or detailed list.
...
- Avoid capability contract overfitting: model general user goals with normalized domain inputs rather than turning one example phrase or circumstance into a product limitation. Treat examples as representative unless they document an intentional constraint, and extend the normalized contract instead of enumerating paraphrases or relative expressions. Use bounded metadata values, typed partial clarifications, and opaque recent-result references where those concepts make the semantic contract explicit.
...
- Streaming speech timeouts measure provider inactivity while connecting or awaiting the next body chunk; disarm the timeout while a yielded chunk is under consumer processing so valid long speech is not treated as a stalled provider.
- Bound best-effort streaming speech reader cancellation so a non-settling cleanup promise cannot hide the primary timeout or abort outcome.
...
- Realtime transcription must cap websocket payloads, individual and cumulative transcript text, audio chunk and stream bytes, await socket send flow control, and bound producer cleanup so a non-settling iterator cannot hide the primary failure.
- The Google Calendar adapter is read-only, selected with `features.calendar.adapter: "google"`, defaults generic upcoming event searches to `features.calendar.upcomingWindowDays: 14`, and must keep OAuth credentials in environment variables such as `GOOGLE_CALENDAR_ACCESS_TOKEN` or `GOOGLE_CALENDAR_CLIENT_ID`, `GOOGLE_CALENDAR_CLIENT_SECRET`, and `GOOGLE_CALENDAR_REFRESH_TOKEN`.
- Calendar titles must be emoji-free at the application boundary. The optional narrow event-grouping provider defaults to the configless deterministic mock, receives only events from dates with multiple entries while preserving their original result indexes, and must return adapter-validated typed groups. It may combine only clearly connected same-day entries, selecting at most four important milestones while retaining every original event reference and exact fact; invalid or failed grouping falls back to the ungrouped result with internal diagnostics.
...
- Treat a runtime-owned background task that returns before shutdown as fatal; bound task joins and shutdown hooks so non-settling cleanup is logged without preventing later lifecycle cleanup.
...
- Spike 9 is complete as historical future-milestone discovery; keep `docs/08-spike-9-report.md` aligned with the implemented Milestones 10 through 12.1 and clearly mark superseded recommendations.
- Milestones 13 through 16 are implemented after their fresh thermonuclear review findings were addressed and the full validation gate passed. Milestone 17 remains planned: daily briefings. Keep future considerations unnumbered until they receive explicit scope and acceptance criteria.
- Personal profile state must contain only explicit user-authored facts in its first milestone, preserve provenance, support explanation/correction/deletion, and expose narrow requested fields rather than injecting the whole profile into providers or features. Text and voice requests such as “set my name to Zak” must become decoded, validated profile commands; provider or conversation memory must never persist the fact directly.
- When any selected capability depends on an absent personal profile fact, use the generic narrow profile-read workflow: ask with explicit save disclosure, validate and save the exact reply through the application-owned profile command, then resume the selected capability. Do not encode target-specific prompt cases. Changed-topic replies must abandon the save, and application-owned save-before-resume ordering must not be delegated to provider output.
- Treat internet search results and extracts as untrusted data. Keep search bounded and read-only, validate all returned citation annotations, project excess valid sources into the configured result limit without retaining claims supported only by excluded citations, require retained citations to resolve to the current returned source set, and never interpret retrieved content as commands, permissions, or confirmation policy.
- Keep internet-search URLs in validated citation/result-reference metadata. One application-owned human-text policy must sanitize answers, titles, and extracts before speech or result-reference retention. Human text and speech use natural source titles without raw URLs, Markdown links, citation brackets, or duplicated source lists; hyperlink-capable text boundaries may attach the exact URL as a hidden source-title link target through a one-pass non-overlapping renderer.
- OpenAI intent fallback, clarification, conversation, and response-rewriter parsers must enforce spoken-text safety after prompting; prompt compliance alone is not a human-boundary guarantee.
- Keep one application-owned spoken-text policy across assistant, tool-observation, CLI, notification, and TTS boundaries. Prompt models to use natural local dates, times, timezone labels, and source titles, but deterministically prevent raw URLs, Markdown link targets, ISO/RFC timestamps, and validated IANA identifiers from reaching human output while preserving exact structured facts and citation URLs.
- Feature results that express subject-local temporal facts must declare their spoken-text timezone and date style explicitly; core must not infer rendering context from arbitrary parameter names. Calendar retains its explicit UTC-day wording mode, while weather uses the selected location timezone.
- Neutral service composition must decorate every notification delivery with the shared spoken-text policy. Voice adapters receive already-safe notification text and must not make enforcement optional.
- OpenAI intent semantics must fail safely when a required string parameter merely echoes the whole request or a narrow action question is misclassified as a broad capability-list request; return one canonical clarification instead of executing the provider's guess. Intent providers must omit absent optional parameters rather than asking for information the capability does not require.
- Weather answers and watches must preserve exact location, timezone, units, forecast period, and freshness data. A stored home location must be explicit, and weather watches are convenience notifications rather than guaranteed emergency alerts. Milestone 15 uses Open-Meteo's free non-commercial forecast and geocoding APIs without a key, must provide required attribution, and must not add a weather credential or paid endpoint without an explicit later decision.
- Default weather speech should lead with temperature and conditions, mention only materially notable rain or wind qualitatively with explicit current-versus-forecast tense, and omit exact precipitation, wind, and fetch telemetry unless explicitly requested; retain every exact fact in structured result data and describe observation age relative to the live clock.
- Weather geocoding must preserve provider rank and safe matching metadata separately from the selected forecast location. Low-risk reads may deterministically use the highest-ranked exact candidate while persistent weather-watch actions require a unique or explicitly country-qualified match.
- Weather clothing advice must use the separately configured narrow adviser provider. Support arbitrary named-item assessment and open outfit recommendation without a clothing-category enum; validate bounded structured output, skip general response rewriting, and preserve adviser failures as safe feature responses with internal diagnostics.
- Weather-watch stores admit at most 24 active watches. Evaluation shares one forecast across identical location/period requests, limits independent provider work to four concurrent requests, stops queued work on shutdown, and isolates one group or watch failure from the rest of the cycle.
- Keep task/reminder lifecycle separate from alarm lifecycle. Reminder delivery must use the exact composed task store, persist claims before output, survive restart without duplicate delivery, and never silently mark its task complete.
- Daily briefings must aggregate a fixed set of narrow read ports through application-owned orchestration; they must not invoke feature plugins, widen provider-directed tool limits, or let source content add actions or sections.
- Bounded tool-chain workflows may execute at most two sequential capabilities explicitly marked `toolChain: "read"`, followed by one terminal command or existing one-to-three-step plan. Parallel calls, intermediate writes, provider-directed retry after failure, and arbitrary loops fail closed.
- Execute intermediate tool-chain reads without response rewriting; rewriting is final human-response post-processing only.
- Keep bounded intent orchestration in the core-owned `IntentWorkflow` transaction. Provider session failures must return a safe diagnostic-aware outcome and preserve every completed read's metadata.
- Tool observations contain only bounded safe response text, an explicit minimal scalar projection, and opaque public references. Never implicitly expose broad feature result data, diagnostics, credentials, raw provider payloads, private result targets, or provider IDs through the intent session; fail closed before continuation when the serialized observation exceeds its application limit.
- Keep one process-local pending interaction per assistant instance: confirmation or one resumable clarification. No/cancel discards a clarification; an answer resumes the exact provider session; a provider-classified changed-topic reply or a terminal continuation that no longer contains the selected clarification capability discards that session and starts one fresh workflow from the exact trusted reply; a resulting validated confirmation replaces the clarification. Confirmed execution must remain in its originating intent workflow so a later feature clarification can resume the same provider session. Open rephrase prompts request another voice turn without creating pending clarification state. Core owns transition legality, passes only typed safe original-request/prompt/origin/capability/parameter context, and restarts provider transport when semantic validation clarifies an outstanding tool call.
- A single feature command that returns the discriminated resumable clarification result must identify the parameter being requested and register that prompt through the same core-owned clarification session with typed `feature_execution` origin; `expectsFollowUp` alone remains only a human/voice capture hint. Compound plans and intermediate tool-chain reads fail closed on feature clarification rather than treating the prompt as success.
- Calendar-driven alarms are confirmed snapshots. Freeze event title/start, offset, timezone, label, and final alarm instant; persist no calendar provider ID and do not silently track event edits or deletion. All-day events require one explicit local-time clarification.
- OpenAI bounded intent continuation uses `previous_response_id`, strict read tools, and disabled parallel calls. Document that this depends on provider-managed stored response state and the operator's OpenAI retention/privacy settings.
- Require every OpenAI Responses API intent response to include a non-empty response ID at the adapter's external-data boundary.
- Milestone 12.1 is implemented after its fresh thermonuclear review findings were addressed and the full validation gate passed. The unimplemented provider-focused roadmap that followed it was retired on 2026-07-28 and replaced by capability-focused Milestones 13 through 17.
...
- Calendar result follow-ups retain only the latest process-local result set, capped at ten opaque event references; a new result replaces it, three subsequent completed assistant turns expire it, and conversation compaction clears references from earlier turns without erasing a result set first displayed by that same compacting turn.
...
- Voice benchmark capture must select explicit PulseAudio input and output devices when `PULSE_SERVER` is present for WSLg, while native Linux and Raspberry Pi hosts retain their configured SoX default devices; child processes receive only the required audio environment.
- Voice benchmark external-data parsers must use the benchmark-local structural parsing primitives for shared record, array, string, stable-ID, digest, and integer checks while keeping schema-specific validation local.
- Third-party voice models and engines must come directly from approved official GitHub or Hugging Face upstreams at immutable revisions, remain in cooling-off for at least 30 full days, and have exact byte counts and SHA-256 values verified before extraction or use. Exact Python runtime dependencies may use immutable official PyPI wheel URLs, but model and engine payloads may not; install wheels with indexes and dependency resolution disabled and allowlist every transitive dependency explicitly. Reject absolute paths, parent traversal, hard links, and symbolic links before extracting archives; install into new private versioned directories without overwriting, never pass credentials, run offline with a minimal environment, and never perform implicit downloads or upgrades. Executable/model artifacts require an upstream-published SHA-256; a human-readable companion config may use a documented immutable-revision inspection plus a locally locked SHA-256.
...
- `npm run test:e2e:openai:clarifications` - run the focused opt-in live OpenAI open-rephrase and changed-topic clarification smoke with `.env`, including exact request-topology assertions.
