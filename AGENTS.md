...
- Third-party voice models and engines must come directly from approved official GitHub or Hugging Face upstreams at immutable revisions, remain in cooling-off for at least 30 full days, and have exact byte counts and SHA-256 values verified before extraction or use. Reject absolute paths, parent traversal, hard links, and symbolic links before extracting archives; install into new private versioned directories without overwriting, never pass credentials, run offline with a minimal environment, and never perform implicit downloads or upgrades. Executable/model artifacts require an upstream-published SHA-256; a human-readable companion config may use a documented immutable-revision inspection plus a locally locked SHA-256.
