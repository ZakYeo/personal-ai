...
- `npm run benchmark:voice:capture -- --speaker primary` - interactively capture only missing core personal voice benchmark phrases through SoX, retain one second of trailing silence, and require consent before promotion; add `--all` for extended phrases.
