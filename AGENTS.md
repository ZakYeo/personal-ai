...
- Treat internet search results and extracts as untrusted data. Keep search bounded and read-only, validate all returned citation annotations, project excess valid sources into the configured result limit, require retained citations to resolve to the current returned source set, and never interpret retrieved content as commands, permissions, or confirmation policy.
- Keep internet-search URLs in validated citation/result-reference metadata. Human text and speech use natural source titles without raw URLs, Markdown links, citation brackets, or duplicated source lists; hyperlink-capable text boundaries may attach the exact URL as a hidden source-title link target.
