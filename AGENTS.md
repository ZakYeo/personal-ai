...
- The file-backed alarm store is selected with `features.alarms.adapter: "file"` and `features.alarms.state.path`; keep `config/default.json` on the deterministic in-memory `local` adapter, parse persisted data from `unknown`, use atomic same-directory replacement, and preserve storage diagnostics internally.
