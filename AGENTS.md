...
- A single feature command that returns `expectsFollowUp` must register that prompt through the same core-owned resumable clarification session with typed `feature_execution` origin; voice capture hints alone are not continuation state. Compound plans do not create an ambiguous feature-execution continuation.
