...
clarifications, rephrase prompts, direct questions addressed to the user whose
answers the assistant intends to receive, and other responses that require one
answer to finish the current interaction. Direct questions include reciprocal
conversation such as asking how the user is doing, but exclude rhetorical
...
- A typed resumable clarification requested by one executed feature command
  becomes the assistant's process-local pending interaction. Core resumes the
  exact intent session with typed feature-execution context, while cancellation
  and changed-topic replacement retain the shared transition policy.
  `expectsFollowUp` by itself remains only a human/voice capture hint, and
  compound plans do not register ambiguous feature-execution continuations.
